...
function isImageLoaded(imageSrc) {
  return (
    imageSrc
    && imageSrc in imageCache
    && imageCache[imageSrc].loaded
  );
}
...
class ReactImageLightbox extends Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (sourcesChanged || this.moveRequested) {
      ...
      Object.keys(prevSrcDict).forEach((prevSrc) => {
        if (!(prevSrc in nextSrcDict) && prevSrc in imageCache) {
          imageCache[prevSrc].loaded = false;
        }
      });
  ...
  getBestImageForType(srcType) {
    ...
    if (isImageLoaded(imageSrc)) {
      ...
      fitSizes = this.getFitSizes(
        imageCache[imageSrc].width,
        imageCache[imageSrc].height,
      );
    } else if (isImageLoaded(this.props[`${srcType}Thumbnail`])) {
    ...
    return {
      src: imageSrc,
      height: imageCache[imageSrc].height,
      width: imageCache[imageSrc].width,
  ...
  handleMoveStart({ x: clientX, y: clientY }) {
    ...
    this.moveStartOffsetX = offsetX;
    this.moveStartOffsetY = offsetY;
  }
  ...
  handleSwipeEnd(event) {
    ...
    const { prevSrc, nextSrc } = this.props;
    if (xDiff > 0 && prevSrc) {
      event.preventDefault();
  ...
  async loadAllImages(props = this.props) {
    ...
    // Load the images
    const promises = this.getSrcTypes().map(async (srcType) => {
      const type = srcType.name;

      // there is no error when we try to load it initially
      if (this.props[type] && this.state.loadErrorStatus[type]) {
        this.setState((prevState) => ({
