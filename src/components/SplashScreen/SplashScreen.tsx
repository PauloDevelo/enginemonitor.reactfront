...
const SplashScreen = ({ isAppInitializing }: Props) => {
  ...
  const [leftSegVis, setLeftSegVis] = useState<boolean>(false);
  const [rightSegVis, setRightSegVis] = useState<boolean>(false);
  const [visible, setVisible] = useState(true);

  useEffect(() => {
    if (isAppInitializing === true && previousIsAppInitializing.current === false) {
      setLeftSegVis(false);
      setRightSegVis(false);
      setVisible(true);
    }

    if (isAppInitializing === false) {
      setTimeout(() => setLeftSegVis(true), 500);
      setTimeout(() => setRightSegVis(true), 800);
  ...
  if (visible) {
    return (
      <div className="overlay">
        ...
        >
          <g>
            <g fill="none" fillRule="evenodd" stroke="#7391bd" className={isAppInitializing ? '' : 'fadeout'} transform="translate(430, 490)">
              <g strokeWidth="16">
            ...
            <path
              id="polygon27"
              className={leftSegVis ? 'fadein' : 'fadeout'}
              style={{ display: 'inline', fill: '#7391bd' }}
              d="M 493.68089,607.41302 458.488,642.399 381.939,565.863 415.248,528.835 Z"
            />
            <path
              id="path46"
              className={rightSegVis ? 'fadein' : 'fadeout'}
